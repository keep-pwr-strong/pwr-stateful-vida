...
pub fn test_merkle_tree() -> Result<()> {
    println!("=== Rust Merkle Tree Verification Test ===");
    
    let test_tree_name = "verification_test".to_string();
    let test_path = format!("merkleTree/{}", test_tree_name);
    
    let tree = MerkleTree::new(test_tree_name.clone()).map_err(|e| std::io::Error::other(format!("{:?}", e)))?;
    
    println!("1. Initial empty tree:");
    println!("   Leaves: {}", tree.get_num_leaves().map_err(|e| std::io::Error::other(format!("{:?}", e)))?);
    println!("   Depth: {}", tree.get_depth().map_err(|e| std::io::Error::other(format!("{:?}", e)))?);
    let root_hash = tree.get_root_hash().map_err(|e| std::io::Error::other(format!("{:?}", e)))?;
    println!("   Root hash: {}", 
        root_hash.as_ref().map(|h| to_hex_string(h)).unwrap_or("null".to_string()));
    
    // Test 1: Add first key-value pair
    let key1 = b"test_key_1";
    let data1 = b"test_data_1";
    tree.add_or_update_data(key1, data1).map_err(|e| std::io::Error::other(format!("{:?}", e)))?;
    
    println!("\n2. After adding first key-value pair:");
    println!("   Key: {}", to_hex_string(key1));
    println!("   Data: {}", to_hex_string(data1));
    println!("   Leaves: {}", tree.get_num_leaves().map_err(|e| std::io::Error::other(format!("{:?}", e)))?);
    println!("   Depth: {}", tree.get_depth().map_err(|e| std::io::Error::other(format!("{:?}", e)))?);
    let root1 = tree.get_root_hash().map_err(|e| std::io::Error::other(format!("{:?}", e)))?;
    println!("   Root hash: {}", 
        root1.as_ref().map(|h| to_hex_string(h)).unwrap_or("null".to_string()));
    
    // Test 2: Add second key-value pair
    let key2 = b"test_key_2";
    let data2 = b"test_data_2";
    tree.add_or_update_data(key2, data2).map_err(|e| std::io::Error::other(format!("{:?}", e)))?;
    
    println!("\n3. After adding second key-value pair:");
    println!("   Key: {}", to_hex_string(key2));
    println!("   Data: {}", to_hex_string(data2));
    println!("   Leaves: {}", tree.get_num_leaves().map_err(|e| std::io::Error::other(format!("{:?}", e)))?);
    println!("   Depth: {}", tree.get_depth().map_err(|e| std::io::Error::other(format!("{:?}", e)))?);
    let root2 = tree.get_root_hash().map_err(|e| std::io::Error::other(format!("{:?}", e)))?;
    println!("   Root hash: {}", 
        root2.as_ref().map(|h| to_hex_string(h)).unwrap_or("null".to_string()));
    
    // Test 3: Add third key-value pair
    let key3 = b"test_key_3";
    let data3 = b"test_data_3";
    tree.add_or_update_data(key3, data3).map_err(|e| std::io::Error::other(format!("{:?}", e)))?;
    
    println!("\n4. After adding third key-value pair:");
    println!("   Key: {}", to_hex_string(key3));
    println!("   Data: {}", to_hex_string(data3));
    println!("   Leaves: {}", tree.get_num_leaves().map_err(|e| std::io::Error::other(format!("{:?}", e)))?);
    println!("   Depth: {}", tree.get_depth().map_err(|e| std::io::Error::other(format!("{:?}", e)))?);
    let root3 = tree.get_root_hash().map_err(|e| std::io::Error::other(format!("{:?}", e)))?;
    println!("   Root hash: {}", 
        root3.as_ref().map(|h| to_hex_string(h)).unwrap_or("null".to_string()));
    
    // Test 4: Update first key
    let data1_updated = b"updated_data_1";
    tree.add_or_update_data(key1, data1_updated).map_err(|e| std::io::Error::other(format!("{:?}", e)))?;
    
    println!("\n5. After updating first key:");
    println!("   Updated data: {}", to_hex_string(data1_updated));
    println!("   Leaves: {}", tree.get_num_leaves().map_err(|e| std::io::Error::other(format!("{:?}", e)))?);
    println!("   Depth: {}", tree.get_depth().map_err(|e| std::io::Error::other(format!("{:?}", e)))?);
    let root4 = tree.get_root_hash().map_err(|e| std::io::Error::other(format!("{:?}", e)))?;
    println!("   Root hash: {}", 
        root4.as_ref().map(|h| to_hex_string(h)).unwrap_or("null".to_string()));
    
    // Test 5: Verify data retrieval
    println!("\n6. Data retrieval verification:");
    let retrieved1 = tree.get_data(key1).map_err(|e| std::io::Error::other(format!("{:?}", e)))?;
    let retrieved2 = tree.get_data(key2).map_err(|e| std::io::Error::other(format!("{:?}", e)))?;
    let retrieved3 = tree.get_data(key3).map_err(|e| std::io::Error::other(format!("{:?}", e)))?;
    println!("   Key1 data: {}", 
        retrieved1.as_ref().map(|d| to_hex_string(d)).unwrap_or("null".to_string()));
    println!("   Key2 data: {}", 
        retrieved2.as_ref().map(|d| to_hex_string(d)).unwrap_or("null".to_string()));
    println!("   Key3 data: {}", 
        retrieved3.as_ref().map(|d| to_hex_string(d)).unwrap_or("null".to_string()));
    
    // Test 6: Calculate individual leaf hashes for verification
    println!("\n7. Individual leaf hashes (for comparison):");
    let leaf_hash1 = calculate_leaf_hash(key1, data1_updated);
    let leaf_hash2 = calculate_leaf_hash(key2, data2);
    let leaf_hash3 = calculate_leaf_hash(key3, data3);
    println!("   Leaf1 hash (key1 + updated_data1): {}", to_hex_string(&leaf_hash1));
    println!("   Leaf2 hash (key2 + data2): {}", to_hex_string(&leaf_hash2));
    println!("   Leaf3 hash (key3 + data3): {}", to_hex_string(&leaf_hash3));
    
    // Test 7: Test with simple known values
    println!("\n8. Simple test with known values:");
    let simple_key = b"hello";
    let simple_data = b"world";
    let simple_hash = calculate_leaf_hash(simple_key, simple_data);
    println!("   SHA256('hello' + 'world'): {}", to_hex_string(&simple_hash));
    
    // Clean up
    tree.close().map_err(|e| std::io::Error::other(format!("{:?}", e)))?;
